...
from rewlis.controller.audio import AudioClass as audio
from rewlis.controller.recognizer import RecognizerClass as recognizer
    ...
    def process(self):
        for book in self.folder_of_books:
            ...
            if os.path.exists(f"{self.data}/{book}/{self.config.VALID}"):
                with open(f"{self.data}/{book}/{self.config.VALID}", mode="r", encoding="UTF-8") as f:
                    valid = f.read()
                if valid == "True":
                    print(f"Book {book} is valid, continue...")
                    continue
            if not os.path.exists(f"{self.data}/{book}/{self.config.RUS_TXT}"):
                print(f"Файла {self.data}/{book}/{self.config.RUS_TXT} не существует")
                sys.exit()
            with open(f"{self.data}/{book}/{self.config.RUS_TXT}", mode="r", encoding="UTF-8") as rus:
                rus_txt = rus.read()
            if not os.path.exists(f"{self.data}/{book}/{self.config.ENG_TXT}"):
                print(f"Файла {self.data}/{book}/{self.config.ENG_TXT} не существует")
                sys.exit()
            with open(f"{self.data}/{book}/{self.config.ENG_TXT}", mode="r", encoding="UTF-8") as eng:
                eng_txt = eng.read()
            if not os.path.exists(os.getcwd() + f"/{self.data}/{book}/" + self.config.RUS_FLAC):
                mp3rus = [f"{self.data}/{book}/mp3rus/{x}" for x in os.listdir(f"{self.data}/{book}/mp3rus") if x[-4:] == ".mp3"]
                print(mp3rus)
                audio.AudioClass(audio_list=mp3rus, output=os.getcwd() + f"/{self.data}/{book}", language="rus")
            if not os.path.exists(os.getcwd() + f"/{self.data}/{book}/" + self.config.ENG_FLAC):
                mp3eng = [f"{self.data}/{book}/mp3eng/{x}" for x in os.listdir(f"{self.data}/{book}/mp3eng") if x[-4:] == ".mp3"]
                print(mp3eng)
                audio.AudioClass(audio_list=mp3eng, output=os.getcwd() + f"/{self.data}/{book}", language="eng")

            del_me = [f"{self.data}/{book}/{x}.bat" for x in ["audio", "wav", "flac"]]
            for d in del_me:
                if os.path.exists(d):
                    os.remove(d)
            ...
            if not os.path.exists(f"{self.data}/{book}/{self.config.RUS_SYNC}"):
                with open(f"{self.data}/{book}/rus.map.json", mode="r", encoding="UTF-8") as map_json:
                ...
                if not os.path.exists(f"{self.data}/{book}/rus.html"):
                    with open(f"{self.data}/{book}/rus.html", mode="w", encoding="UTF-8") as f:
                        f.write(rus_html)
                ...
                with open(f"{self.data}/{book}/{self.config.RUS_SYNC}", mode="r") as fsync:
                    sync2 = json.load(fsync)

            if not os.path.exists(f"{self.data}/{book}/{self.config.ENG_SYNC}"):
                with open(f"{self.data}/{book}/eng.map.json", mode="r", encoding="UTF-8") as map_json:
                    R_start, R_end, R_word = r_map(json.load(map_json))
                eng_html = text2html(text=eng_txt.lower(),
                                     pattern=r'([а-я0-9a-z]+)([^а-я0-9a-z]+)')
                if not os.path.exists(f"{self.data}/{book}/eng.html"):
                    with open(f"{self.data}/{book}/eng.html", mode="w", encoding="UTF-8") as f:
                        f.write(eng_html)
                synchronize, L_word, L_start, L_end = cross.get_sim(eng_html, R_word)
                sync_eng = sync.Sync(output=f"{self.data}/{book}", language="eng")
                ...
                with open(f"{self.data}/{book}/{self.config.ENG_SYNC}", mode="r") as fsync:
            ...
            if not os.path.exists(f"{self.data}/{book}/{self.config.RUS_ORIG}"):
                ...
                with open(f"{self.data}/{book}/{self.config.RUS_ORIG}", mode="w", encoding="UTF-8") as f:
                    f.write(orig_html)
            ...
            if not os.path.exists(f"{self.data}/{book}/{self.config.ENG_ORIG}"):
                orig_html2 = text2html(text=eng_txt.lower(),
                                       pattern=r'(([а-я0-9a-z]+[^а-я0-9a-z]+){4})',
                                       replacepattern=r'<p>\1</p>')
                with open(f"{self.data}/{book}/{self.config.ENG_ORIG}", mode="w", encoding="UTF-8") as f:
                    f.write(orig_html2)

            sync_rus = sync.Sync(output=f"{self.data}/{book}", language="rus")
            if not os.path.exists(f"{self.data}/{book}/two.json"):
                print("Not find file two.json, creating...")
                synchronize, L_word, R_word, L_end, R_end = cross.get_sim_v2(book, self.data)
            ...
            micro = []
            if not os.path.exists(f"{self.data}/{book}/{self.config.MICRO_JSON}"):
                for i in two_sync:
                    phraza_1 = i[2]
                    phraza_2 = i[3]
                ...
                with open(f"{self.data}/{book}/{self.config.MICRO_JSON}", mode="w") as f:
                    f.write(json_string)

            micro2 = []
            print("Load file micro.json")
            with open(f"{self.data}/{book}/{self.config.MICRO_JSON}", mode="r") as f:
                m = json.load(f)
            ...
            with open(f"{self.data}/{book}/{self.config.VALID}", mode="w", encoding="UTF-8") as f:
                f.write("True")


if __name__ == "__main__":
    model = Model()
    create = Creator(model=model)
    create.init()
    create.process()
