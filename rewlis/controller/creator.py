    ...
    def create(self):
        ...
        for book in folder_of_books:
            if not os.path.isdir(f"{data}/{book}"):
                continue
            if os.path.exists(f"{data}/{book}/{self.config.VALID}"):
                with open(f"{data}/{book}/{self.config.VALID}", mode="r", encoding="UTF-8") as f:
                    valid = f.read()
                if valid == "True":
                    print(f"Book {book} is valid, continue...")
                    continue
            if not os.path.exists(f"{data}/{book}/{self.config.RUS_TXT}"):
                print(f"Файла {data}/{book}/{self.config.RUS_TXT} не существует")
                sys.exit()
            with open(f"{data}/{book}/{self.config.RUS_TXT}", mode="r", encoding="UTF-8") as rus:
                rus_txt = rus.read()
            if not os.path.exists(f"{data}/{book}/{self.config.ENG_TXT}"):
                print(f"Файла {data}/{book}/{self.config.ENG_TXT} не существует")
                sys.exit()
            with open(f"{data}/{book}/{self.config.ENG_TXT}", mode="r", encoding="UTF-8") as eng:
                eng_txt = eng.read()
            if not os.path.exists(os.getcwd() + f"/{data}/{book}/" + self.config.RUS_FLAC):
            ...
            if not os.path.exists(os.getcwd() + f"/{data}/{book}/" + self.config.ENG_FLAC):
            ...
            if not os.path.exists(f"{data}/{book}/{self.config.RUS_SYNC}"):
                ...
                with open(f"{data}/{book}/{self.config.RUS_SYNC}", mode="r") as fsync:
                    sync2 = json.load(fsync)

            if not os.path.exists(f"{data}/{book}/{self.config.ENG_SYNC}"):
                ...
                with open(f"{data}/{book}/{self.config.ENG_SYNC}", mode="r") as fsync:
                    sync1 = json.load(fsync)

            if not os.path.exists(f"{data}/{book}/{self.config.RUS_ORIG}"):
                orig_html = text2html(text=rus_txt.lower(),
                                      pattern=r'(([а-я0-9a-z]+[^а-я0-9a-z]+){4})',
                                      replacepattern=r'<p>\1</p>')
                with open(f"{data}/{book}/{self.config.RUS_ORIG}", mode="w", encoding="UTF-8") as f:
                    f.write(orig_html)

            if not os.path.exists(f"{data}/{book}/{self.config.ENG_ORIG}"):
                orig_html2 = text2html(text=eng_txt.lower(),
                                       pattern=r'(([а-я0-9a-z]+[^а-я0-9a-z]+){4})',
                                       replacepattern=r'<p>\1</p>')
                with open(f"{data}/{book}/{self.config.ENG_ORIG}", mode="w", encoding="UTF-8") as f:
                    f.write(orig_html2)
            ...
            micro = []
            if not os.path.exists(f"{data}/{book}/{self.config.MICRO_JSON}"):
                for i in two_sync:
                    phraza_1 = i[2]
                    phraza_2 = i[3]
                ...
                with open(f"{data}/{book}/{self.config.MICRO_JSON}", mode="w") as f:
                    f.write(json_string)

            micro2 = []
            print("Load file micro.json")
            with open(f"{data}/{book}/{self.config.MICRO_JSON}", mode="r") as f:
                m = json.load(f)
            ...
            with open(f"{data}/{book}/micro2.json", mode="w") as f:
                f.write(json_string)

            if not os.path.exists(f"{data}/{book}/eng2rus.json"):
                eng2rus = eng_to_rus(micro2, R_POS, L_POS, sync1, sync2)
                json_string = json.dumps(eng2rus)
                print("Save to eng2rus.json")
                with open(f"{data}/{book}/eng2rus.json", mode="w") as f:
                    f.write(json_string)

            if not os.path.exists(f"{data}/{book}/rus2eng.json"):
                rus2eng = eng_to_rus(micro2, L_POS, R_POS, sync2, sync1)
                json_string = json.dumps(rus2eng)
                print("Save to rus2eng.json")
                with open(f"{data}/{book}/rus2eng.json", mode="w") as f:
                    f.write(json_string)

            with open(f"{data}/{book}/{self.config.VALID}", mode="w", encoding="UTF-8") as f:
                f.write("True")
