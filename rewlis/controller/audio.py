...
from rewlis.entity import ALL_MP3, ALL_FLAC
...
class Audio:
    def __init__(self, cprint, audio_list, output, language):
        ...
        self.MP3 = f"{self.output}/{ALL_MP3}"
        self.FLAC = f"{self.output}/{ALL_FLAC}"
    ...
    def create_chunk(self):
        ...
        if not exists(f"{self.output}/chunk"):
            os.mkdir(f"{self.output}/chunk")
        self.CHUNK = [
            f"{self.output}/chunk" +
    ...
    def create_wav(self):
        ...
        cbn = sox.Transformer()
        cbn.convert(samplerate=16000, n_channels=1)
        if not exists(f"{self.output}/wav"):
            os.mkdir(f"{self.output}/wav")
        self.WAV = [
            f"{self.output}/wav" +
